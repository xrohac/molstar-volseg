...
def _create_category_set_downsamplings_gpu(
    ...
    levels.pop(0)
    print(f"Levels before filtering: {[lvl.get_ratio() for lvl in levels]}")
    print(f"Ratios to be stored: {ratios_to_be_stored}")
    print(f"Levels before filtering: {len(levels)}")

    levels = [lvl for lvl in levels if lvl.get_ratio() in ratios_to_be_stored]
    print(f"Levels after filtering: {len(levels)}")
    print(f"Levels after filtering: {[lvl.get_ratio() for lvl in levels]}")
    print(f"Ratios to be stored: {ratios_to_be_stored}")
